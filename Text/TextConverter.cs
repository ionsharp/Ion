...
{
    [Name(nameof(Action))]
    public record class Action : Lockable
    {
        public enum Types
        {
            Decrypt,
        ...
        public override string ToString(string format, IFormatProvider provider)
            => Target == Targets.None ? "Copy" : $"{Type} {Target.ToString().GetCamel().ToLower()}";
    }
    ...
    public static string EscapeUri(string uri, Action.Types actionType)
    {
        switch (actionType)
        {
            case Action.Types.Decrypt:
                uri = uri.Replace('-', '/');
                return System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(uri));
            case Action.Types.Encrypt:
                uri = System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(uri));
                return uri.Replace('/', '-');
        }
        return uri;
    }
    ...
    public static string FormatUri(string uriHead, string uriTail, ItemType targetType, Action action)
    {
        ...
        var pieces = uriTail.Split((char[])['\\'], StringSplitOptions.RemoveEmptyEntries);
    ...
    public static string DecryptText(string text, string password, SymmetricAlgorithm algorithm = SymmetricAlgorithm.Rijndael, Encoding encoding = Encoding.Unicode)
    {
        ...
        return encoding.New().GetString(result);
    }
    ...
    public static string EncryptText(string text, string password, SymmetricAlgorithm algorithm = SymmetricAlgorithm.Rijndael, Encoding encoding = Encoding.Unicode)
    {
        ...
        //First we need to turn the input string into a byte array; we assume that base64 encoding was used 
        var bytes = encoding.New().GetBytes(text);
    ...
    public class Callback(Func<double, double, bool> action)
    {
        readonly Func<double, double, bool> Action = action;
...
#pragma warning disable IDE0060
    ...
    {
        if (password.IsEmpty())
            throw new ArgumentNullException(nameof(password), "Password must contain at least one character.");
        ...
        var size = 0d;
        if (action != null)
            size = Convert.ToDouble(new FileInfo(source).Length);
        ...
        do
        {
            ...
            {
                sizeRead += bytesRead;
                var cancel = action.Invoke(Convert.ToDouble(sizeRead), size);
                if (cancel) break;
            }
        }
        while (bytesRead != 0);
